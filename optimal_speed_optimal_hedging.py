...
C_u_j = 300000 
C_h_j = 0 # loading operation, cost of loading, price in the harbour that you pay, cost of pump
c_j_f = 1 # fuel cost USD/ton  498 and 590 Appendix A.
...
def G_star(j,d,r_state):
    ...
    if j==m:
        ...
        res = minimize(G_wrapper, x0, args=[j,d,r_state], method='trust-constr', bounds=[(0,1),(T_min,T_max)], options={'disp': False, 'maxiter': 500})
        mH = res.fun
        best_h_a = res.x[0]
        best_T = res.x[1]
        return mH, best_h_a, best_T
