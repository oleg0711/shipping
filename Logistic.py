...
class CLeg:
    ...
    def FindBestGoodwill(self,
        ...
        if self.LegNr == oJ.LegsPerRoundtrip:
            ...
            self.FutureGoodwill = oRTList[RoundtripNr].FindGoodwillOfNextLeg(LegNr)
        ...
        self.MainBunkerCarried_Tonne = self.MainBunkerConsumption_TonnePerDay * TimeAtSea_Days
        self.LoadingCost_MainBunker_USD = self.MainBunkerCarried_Tonne * oPList[self.PortNr_StartOfLeg].MainBunkerRate_USDperTonne
        self.LegTime_Days = self.TimeLoading_Days + TimeAtSea_Days + self.TimeWaiting_Days + self.TimeUnloading_Days
        self.Profitability_ThisLeg_USD = (self.Revenues_USD - self.UnloadingCosts_USD) * np.exp(-1 * oJ.OpprtCostCapitalRate * (LegTime_Days / 365)) - self.LoadingCost_MainBunker_USD - LoadingCost_AuxFuel_USD - self.LoadingCost_OtherOperations_USD - oJ.DailyHire_USDperDay * 365 * (1 - np.exp(-1 * oJ.OpprtCostCapitalRate * (self.LegTime_Days / 365))) / (oJ.OpprtCostCapitalRate)
       ...
       #If this leg is the first of the roundtrip, store the Goodwill also into this roundtrips goodwill
        if self.LegNr == 1:
            oRTList[self.RoundtripNr].RoundtripGoodwill = Goodwill_USD
...
class CPort:
    ...
    def CalculateCargoImplicationsData(self, oJ, oS, oPList): #list of ports
        ...
        for i in range(0,oJ.LegsPerRoundtrip):
            ...
            if i > 0:
                oPList[i].UnloadingTime_Hrs = oPList[i - 1].CargoIntake_Barrels * oP[i - 1].CargoIntake_QBmetresperBarrel / oS.ShipDischargeRate
            else:
    ...
    def __init__(self, oJ:CJourney, oS:CShip, oPList, TripNr):
        ...
        for i in range(1,1+oJ.LegsPerRoundtrip):
            ...
            oLeg = CLeg(LegNr=i,
                        ...
                        PortNr_EndOfLeg = i,
                        TimeLoading_Days = 0, #oPList[i - 1].LoadingTime_Hrs / 24,
                        TimeWaiting_Days = oPList[i].WaitingTime_Hrs / 24,
                        TimeUnloading_Days = 0, #oPList[i].UnloadingTime_Hrs / 24,
